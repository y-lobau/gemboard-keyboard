...
- When the iOS companion app has a saved Gemini API key, it starts or restores the companion audio session directly from the app lifecycle instead of waiting for keyboard-visibility demand evaluation.
...
- If the iOS host app already marked the companion session active, the iOS keyboard must treat that session as available for handoff immediately instead of reopening the companion app on every microphone press while waiting for the first heartbeat.
...
- If the iOS companion session is interrupted or suspended by another app's audio session, returning to the companion app must retry that companion session automatically so the keyboard can recover without requiring a separate policy engine for keyboard visibility.
...
- If the iOS companion app is already marked active but speech capture still cannot start because the audio engine is unavailable, the keyboard must stay in a retryable error state instead of showing the reopen-app prompt.
