...
- If the host app still needs to prepare microphone access before recording can begin, releasing the control during that preparation must cancel the attempt instead of starting and then immediately stopping a phantom recording.
- While host-app dictation is transcribing on iOS, streamed transcript snapshots must update the live draft progressively instead of waiting for only the final transcript, and those updates must preserve natural spacing between words.
...
- Firebase Remote Config defines seven runtime Gemini values for this app: the LLM model name, the system prompt, and five transcription cost rates named `gemini_cost_input_text`, `gemini_cost_input_audio`, `gemini_cost_input_cache_text`, `gemini_cost_input_cache_audio`, and `gemini_cost_output_text`.
- On app startup, the host app fetches those Firebase Remote Config values on both iOS and Android, persists the model and system prompt into native shared storage for the current platform, and uses the cost rates to render dollar totals in the host-app transcription-cost summary.
...
- If Firebase Remote Config fetch fails, the app and keyboard must keep using the last persisted Gemini runtime configuration and otherwise fall back to the bundled default Gemini model `gemini-3.1-flash-preview` and the bundled default system prompt.
- If Firebase Remote Config returns empty runtime transcription values and no persisted Gemini runtime configuration is available yet, the app must seed native storage with the bundled default Gemini model `gemini-3.1-flash-preview` and the bundled default system prompt so speech capture can still proceed.
