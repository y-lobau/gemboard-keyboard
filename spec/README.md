...
- If the iOS companion app is closed completely and cannot clear its shared state first, the keyboard may keep the session in a retryable recovery state only until the shared companion-presence heartbeat or recovery grace expires; after that timeout it must require the companion app again.
- If the iOS companion app still has the companion session requested active but speech capture cannot start because audio is temporarily unavailable, the keyboard must stay in a retryable recovery state instead of showing the reopen-app prompt.
